...
class SearchBooks extends React.Component {
  ...
  GetImageUrl(detail) {
    return detail.imageLinks !== undefined
  ...
  searchBook(query) {
    ...
    if (query.trim() !== '') {
      BooksAPI.search(query.trim(), 20).then(res => {
        for (let i in res) {
         res[i].shelf = this.props.getBookShelf(res[i].id);
        }
                  ...
                  <div className="book">
                    <div className="book-top">
                      ...
                      <div className="book-shelf-changer">
                        <select
                          onChange={(e) => this.onSelfChange(detail, e.target.value)}
                          value={detail.shelf}>
                    ...
                    <div className="book-authors">{detail.authors !== undefined
                        ? detail
                          .authors
                          .join()
                        : detail.authors}</div>
